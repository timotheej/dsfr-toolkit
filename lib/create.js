...
function update() {
  ...
  // 1. Mise à jour des dépendances npm
  console.log('');
  ...
  try {
    ...
    console.error('');
    console.error('\x1b[31m  ✗ Échec de la mise à jour des dépendances.\x1b[0m');
    console.error('  Essayez manuellement : npm update');
    process.exit(1);
  }

  // 2. Mise à jour du CLAUDE.md depuis le nouveau template
  ...
  if (fs.existsSync(claudePath) && fs.existsSync(templatePath)) {
    ...
    // Générer le nouveau depuis le template
    ...
    console.log('');
    console.log('\x1b[32m  ✓ CLAUDE.md mis à jour (ancien sauvegardé dans CLAUDE.md.bak)\x1b[0m');
  }

  console.log('');
  console.log('  Les fichiers du projet (PROJET.md, pages/) n\'ont pas été touchés.');
  console.log('');
}
