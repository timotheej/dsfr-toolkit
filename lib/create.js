...
function update() {
  ...
  try {
    execSync('npm update @timotheej/dsfr-toolkit @codegouvfr/react-dsfr', {
      cwd: process.cwd(),
      stdio: 'inherit',
    });
    console.log('');
  ...
  // 2. Mise a jour du CLAUDE.md depuis le template
  const claudePath = path.resolve(process.cwd(), 'CLAUDE.md');
  const toolkitRoot = path.resolve(__dirname, '..');
  const templatePath = path.join(toolkitRoot, 'templates', 'CLAUDE.md.react.tpl');
...
function create(projectName) {
  ...
  const toolkitVersion = toolkitPkg.version || '3.0.0';
  ...
  const topTemplates = [
    { src: 'CLAUDE.md.react.tpl', dest: 'CLAUDE.md' },
  ...
  try {
    ...
    execSync('git commit -m "Initial setup via @timotheej/dsfr-toolkit v3"', {
      cwd: projectPath,
      stdio: 'pipe',
    });
  ...
  console.log('');
  console.log('  \x1b[36mExport Figma (optionnel) :\x1b[0m');
  console.log('');
  console.log('    Une fois vos ecrans valides dans le navigateur,');
  console.log('    dites a Claude : "Exporte cette page dans Figma"');
  console.log('    (necessite le MCP Figma + bibliotheque DSFR connectee)');
  console.log('');
}
